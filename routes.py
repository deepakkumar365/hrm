    ...
    if current_user.role == 'Admin':
        ...
        for leave in recent_leaves:
            recent_activities.append({
                ...
                'details': f"{leave.leave_type} leave from {format_date(leave.start_date)}",
...
def employee_view(employee_id):
    ...
    if current_user.role == 'Employee':
        ...
        if not (hasattr(current_user, 'employee_profile') and 
                (current_user.employee_profile.id == employee_id or 
                 employee.manager_id == current_user.employee_profile.id)):
            return render_template('403.html'), 403
    
    # Get recent payslips
    recent_payslips = Payroll.query.filter_by(employee_id=employee_id).order_by(
...
def employee_edit(employee_id):
    ...
    return render_template('employees/form.html', employee=employee, managers=managers)
    ...
    if request.method == 'POST':
        try:
            ...
            for emp_id in selected_employees:
                ...
                total_overtime = sum(record.overtime_hours or 0 for record in attendance_records)
            ...
            db.session.commit()
            flash(f'Generated payroll for {generated_count} employees', 'success')
...
def payroll_payslip(payroll_id):
    ...
    if current_user.role == 'Employee':
        if not (hasattr(current_user, 'employee_profile') and 
                current_user.employee_profile.id == payroll.employee_id):
            return render_template('403.html'), 403
    elif current_user.role == 'Manager':
        if not (hasattr(current_user, 'employee_profile') and 
                payroll.employee.manager_id == current_user.employee_profile.id):
            return render_template('403.html'), 403
...
def attendance_list():
    ...
    if current_user.role == 'Employee' and hasattr(current_user, 'employee_profile'):
        query = query.filter(Attendance.employee_id == current_user.employee_profile.id)
            ...
            if not existing:
                ...
                if action == 'clock_out':
                    ...
                    if existing.clock_in:
                        clock_in_dt = datetime.combine(today, existing.clock_in)
                        ...
                        if existing.break_start and existing.break_end:
                            break_start_dt = datetime.combine(today, existing.break_start)
                            break_end_dt = datetime.combine(today, existing.break_end)
...
def leave_list():
    ...
    # Role-based filtering
    if current_user.role == 'Employee' and hasattr(current_user, 'employee_profile'):
        query = query.filter(Leave.employee_id == current_user.employee_profile.id)
    elif current_user.role == 'Manager' and hasattr(current_user, 'employee_profile'):
        query = query.filter(Employee.manager_id == current_user.employee_profile.id)
...
def claims_list():
    ...
    return render_template('claims/list.html', claims=claims, status_filter=status_filter)
...
def appraisal_list():
    ...
    # Role-based filtering
    if current_user.role == 'Employee' and hasattr(current_user, 'employee_profile'):
        query = query.filter(Appraisal.employee_id == current_user.employee_profile.id)
    elif current_user.role == 'Manager' and hasattr(current_user, 'employee_profile'):
        query = query.filter(Employee.manager_id == current_user.employee_profile.id)
...
def appraisal_create():
    ...
    if request.method == 'POST':
        try:
            ...
            appraisal.employee_id = int(employee_id_str) if employee_id_str else 0
            appraisal.review_period_start = parse_date(request.form.get('review_period_start'))
            appraisal.review_period_end = parse_date(request.form.get('review_period_end'))
            appraisal.performance_rating = int(request.form.get('performance_rating', '0'))
            appraisal.goals_achievement = int(request.form.get('goals_achievement', '0'))
            appraisal.teamwork_rating = int(request.form.get('teamwork_rating', '0'))
            appraisal.communication_rating = int(request.form.get('communication_rating', '0'))
            
            # Calculate overall rating
            ratings = [
                appraisal.performance_rating,
                appraisal.goals_achievement,
                appraisal.teamwork_rating,
                appraisal.communication_rating
            ]
            ...
            appraisal.training_recommendations = request.form.get('training_recommendations')
    ...
    if current_user.role == 'Manager' and hasattr(current_user, 'employee_profile'):
        employees = employees.filter(Employee.manager_id == current_user.employee_profile.id)
...
def compliance_dashboard():
    ...
    deadlines = [
        {'type': 'CPF Submission', 'date': f"14/{current_month + 1 if current_month < 12 else 1}/{current_year if current_month < 12 else current_year + 1}"},
        {'type': 'AIS Filing', 'date': f"31/03/{current_year + 1}"},
        {'type': 'MOM OED', 'date': f"15/{current_month + 1 if current_month < 12 else 1}/{current_year if current_month < 12 else current_year + 1}"}
    ]
...
def compliance_generate(report_type):
    ...
    try:
        ...
        if report_type == 'cpf':
            ...
            headers = ['Employee ID', 'Name', 'NRIC', 'CPF Account', 'Gross Salary', 
            ...
            headers = ['Employee ID', 'Name', 'NRIC', 'Annual Income', 'Employment Period']
            ...
            headers = ['Employee ID', 'Name', 'Passport/ID', 'Work Permit Type', 
            ...
            headers = ['Employee ID', 'Name', 'Bank Account', 'Bank Name', 'Amount', 'Reference']
        ...
        flash(f'Error generating {report_type.upper()} report: {str(e)}', 'error')
        return redirect(url_for('compliance_dashboard'))
...
def export_employees():
    ...
    headers = ['Employee ID', 'First Name', 'Last Name', 'Email', 'NRIC', 
              'Position', 'Department', 'Hire Date', 'Employment Type', 
              'Work Permit Type', 'Basic Salary']
...
def api_attendance_check():
    ...
    if attendance:
        return jsonify({
            ...
            'clock_in_time': attendance.clock_in.strftime('%H:%M') if attendance.clock_in else None,
            'clock_out_time': attendance.clock_out.strftime('%H:%M') if attendance.clock_out else None
        })
    else:
        return jsonify({
            'clocked_in': False,
