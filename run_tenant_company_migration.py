"""
Run Tenant-Company Hierarchy Migration
...
def run_test_data(conn, filepath):
    """Execute test data SQL file"""
    logger.info(f"Running test data: {filepath}")
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(sql_content)
            conn.commit()
            logger.info(f"✅ Test data inserted: {filepath}")
            return True
    except Exception as e:
        conn.rollback()
        logger.error(f"❌ Test data insertion failed: {filepath}")
        logger.error(f"Error: {str(e)}")
        return False
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("HRMS Tenant-Company Hierarchy Migration")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("STEP 3: Test data insertion (optional)")
        logger.info("=" * 60)
        
        if test_data_file.exists():
            response = input("Do you want to insert test data? (yes/no): ").strip().lower()
            if response in ['yes', 'y']:
                run_test_data(conn, test_data_file)
            else:
                logger.info("Skipping test data insertion")
        else:
        ...
        logger.info("=" * 60)
        logger.info("\nNext steps:")
        logger.info("1. Import the new routes in your main app:")
        logger.info("   from routes_tenant_company import *")
